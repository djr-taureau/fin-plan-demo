...
export class ContentService {
		...
		if (content && content.readableStreamBody) {
			...
			switch (contentMimeType) {
                case 'application/json': return {
                    metadata,
                    content: JSON.parse(<string>(await streamToString(content.readableStreamBody)))
                };
                case 'text/html':
                case 'text/plain':
				default: return {
						metadata,
						content: await streamToString(content.readableStreamBody)
					};
			}
		} else {
	...
	private getContainerName(
		...
		contentType: SystemContentType = SystemContentType.Asset,
		key?: string
	) {
		if (key) {
	...
	private async saveBlob(
		...
		if (headers['content-type'].indexOf('multipart/form-data') >= 0) {
			...
			//if not multipart then content was uploaded raw in req.body
	...
	private async listBlobs(containerName) {
		...
		do {
			...
			for (const blob of listBlobsResponse.segment.blobItems) {
				...
				const content = await this.processReadableStream(
					blobStream,
